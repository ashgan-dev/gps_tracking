...
def xyz(lat,lon,r=EARTH_RADIUS):
    ...
    x = round(int(r*math.cos(math.radians(lat))*math.cos(math.radians(lon))), 2)
    y = round(int(r*math.cos(math.radians(lat))*math.sin(math.radians(lon))), 2)
    z = round(int(r*math.sin(math.radians(lat))), 2)
    return x,y,z
